...
community::~community()
{
  for (size_t i = 0; i < vertices.size(); i++)
    vertices[i] = NULL;
  //delete clonedCommunity;
  clonedCommunity = NULL;
}
...
community* community::clone()
{
  ...
  //new_comm->survived = false;
  clonedCommunity = new_comm;
...
individual::individual(const individual_config& c) throw (nsga2::nsga2exception) :
	rank(0),
	constr_violation(0),
	xreal(0),
	gene(0),
...
void individual::initialize(std::vector<vertex*> verts, int num_comm) throw (nsga2::nsga2exception) 
{
	...
	{
		vertexSet[i] = rgen.integer(1, num_comm);
	}
...
void individual::decode() 
{
	...
	for (int i = 0; i < config->nbin; ++i) 
	{
		...
		xbin[i] = config->limits_binvar[i].first +
			(double)sum*(config->limits_binvar[i].second - 
			config->limits_binvar[i].first) / 
			(double)((1 << (config->nbits[i])) - 1); // TODO: check
	}
}
...
) throw (nsga2::nsga2exception) : crowd_obj(true), ind_config(), eval_pop_function(NULL) 
{
	generation = 1;
...
void population::merge(const population& pop1, const population& pop2) throw (nsga2::nsga2exception)
{
...
void population::report(std::ostream& os) const 
{
	...
	for (it = ind.begin(); it != ind.end(); ++it) 
	{
		for (int j = 0; j < ind_config.nobj; ++j)
			os << it->obj[j] << '\t';
    // TODO: investigate this stuff for figuring out what the numbers mean
		for (int j = 0; j < nvertices; ++j)
			os << it->vertexSet[j] << ' ';
		os << '\n';
...
{
	std::pair<int, int> num_mut = std::make_pair(0, 0), tmp = std::make_pair(0, 0);
	std::vector<individual>::iterator it;
	for (it = ind.begin(); it != ind.end(); ++it) 
	{
		tmp = it->mutate();
