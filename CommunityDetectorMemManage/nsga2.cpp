	...
	individual& child1, individual& child2)
{
  ...
  //vector<community*> commList;
    ...
    //commList.push_back(parent1_commCopy[i]);
		parent1_w_commCopy.push_back(parent1.comm[i]->clone());
    //commList.push_back(parent1_w_commCopy[i]);
	}
    ...
    //commList.push_back(parent2_commCopy[i]);
		parent2_w_commCopy.push_back(parent2.comm[i]->clone());
    //commList.push_back(parent2_w_commCopy[i]);
	}
  ...
  for (size_t vectorsize = 0; vectorsize < biglist.size(); vectorsize++)
  {
    for (size_t vectorsize2 = 0; vectorsize2 < addedElements.size(); vectorsize2++)
    {
      if (vectorsize == addedElements[vectorsize2])
      {
        break;
      }
      else if (vectorsize2 == addedElements.size() - 1)
      {
        delete biglist[vectorsize];
      }
    }
  }
  ...
  for (size_t vectorsize = 0; vectorsize < biglist2.size(); vectorsize++)
  {
    for (size_t vectorsize2 = 0; vectorsize2 < addedElements.size(); vectorsize2++)
    {
      if (vectorsize == addedElements[vectorsize2])
      ...
      {
        delete biglist2[vectorsize];
      }
    }
  }

  /*for (size_t it = 0; it < commList.size(); it++)
  {
    //if (commList[it]->survived == false)
      delete commList[it];
  }*/
}
...
void NSGA2::advance() {
  ...
  parent_pop->ind.clear();
	...
	for (int j = 0; j < extra; ++j) // Pt+1 = Pt+1 U Fi[1:N-|Pt+1|] // TODO: Fix memory leak here
		parent_pop->ind.push_back(mixed_pop->ind[mixed_pop->front[i][j]]);

  vector<vector<size_t>> matches;
  // TODO: Fix this loop up this takes way too long to run
  for (size_t i = 0; i < parent_pop->ind.size(); i++)
  {
    for (size_t j = 0; j < parent_pop->ind.at(i).comm.size(); j++)
    {
      for (size_t k = 0; k < mixed_pop->ind.size(); k++)
      {
        for (size_t l = 0; l < mixed_pop->ind.at(k).comm.size(); l++)
        {
          if (parent_pop->ind.at(i).comm.at(j) == mixed_pop->ind.at(k).comm.at(l))
          {
            vector<size_t> tempMatch;
            tempMatch.push_back(k);
            tempMatch.push_back(l);
  ...
  for (size_t k = 0; k < mixed_pop->ind.size(); k++)
  {
    for (size_t l = 0; l < mixed_pop->ind.at(k).comm.size(); l++)
    {
      ...
      {
        if (k == matches.at(count).at(0) && l == matches.at(count).at(1))
          break;
        else if (count == matches.size() - 1)
          delete mixed_pop->ind.at(k).comm.at(l);
      }
    }
    //cout << "deletes[k]: " << k << endl;
  }
