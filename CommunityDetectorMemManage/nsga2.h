...
namespace nsga2 {
	...
	class NSGA2 
	{
		public:
			...
			void initialize(std::vector<vertex*> verts) throw (nsga2exception);
			...
			void set_pcross_comm(double pcross_comm) 
			{
				this->pcross_comm = pcross_comm;
			};
			...
			void set_pmut_comm(double pmut_comm) 
			{
				this->pmut_comm = pmut_comm;
			};
		...
		public:
			...
			void report_pop_obj(const population& pop, std::ostream& os) const;
			
			void save_backup() const;
			
			bool load_backup();
			
			std::vector<community*> big_list_make
			(
				std::vector<community*> first_list, 
				std::vector<community*> second_list
			);

			void selection(population& oldpop, population& newpop) throw (nsga2::nsga2exception);
