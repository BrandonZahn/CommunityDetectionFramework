                      ...
                      individual& child1, individual& child2)
{
  ...
  std::vector<int> commSize; // <community size> REFINER
  std::vector<int> refinedVertexSet;
  int highest = 1;
  for (int i = 0; i < nvertices; i++)
  {
    commSize.push_back(1);
    refinedVertexSet.push_back(0);
    for (int j = 0; j < nvertices; j++)
    {
      if (child1.vertexSet[i] == child1.vertexSet[j] && i != j)
        commSize[i]++;
    }
    
    if (commSize[i] > highest)
      highest = commSize[i];
  }
  ...
  for (int i = highest; i > 0; i--)
  {
    ...
    { 
      if (commSize[j] == i)
      {
        if (refinedVertexSet[j] == 0)
        {
          refinedVertexSet[j] = commCounter;
          
          for (int k = j + 1; k < nvertices; k++)
          {
            if (child1.vertexSet[k] == child1.vertexSet[j])
              refinedVertexSet[k] = commCounter;
          }
  ...
  child1.vertexSet = refinedVertexSet;
  // END REFINER
  
  commSize.clear(); // <community size> REFINER
  refinedVertexSet.clear();
  highest = 1;
  for (int i = 0; i < nvertices; i++)
  {
    commSize.push_back(1);
    ...
    for (int j = 0; j < nvertices; j++)
    {
      if (child2.vertexSet[i] == child2.vertexSet[j] && i != j)
        commSize[i]++;
    }
    ...
    { 
      if (commSize[j] == i)
      {
        if (refinedVertexSet[j] == 0)
        {
          ...
          for (int k = j + 1; k < nvertices; k++)
          {
            if (child2.vertexSet[k] == child2.vertexSet[j])
              refinedVertexSet[k] = commCounter;
          }
  ...
  child2.vertexSet = refinedVertexSet;
  // END REFINER
}
